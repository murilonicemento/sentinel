...
public class GeospatialHttpClient : IGeospatialClient
{
    ...
    public async Task<bool> IsPointInRiskAreaAsync(int latitude, int longitude, string areaType, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<GeospatialContextDto?> GetSpatialContextAsync(int latitude, int longitude, CancellationToken cancellationToken = default)
    {
        var response = await _httpClient.GetAsync($"/api/geospatial/context?lat={latitude}&lon={longitude}", cancellationToken);
    ...
    public async Task<IReadOnlyList<NearbyRiskDto>> GetNearbyRisksAsync(int latitude, int longitude, double radiusMeters, CancellationToken cancellationToken = default)
    {
        var request = new 
        { 
        ...
        var response = await _httpClient.PostAsJsonAsync("/api/geospatial/query/search-by-radius", request, cancellationToken);
        
        if (!response.IsSuccessStatusCode)
            return new List<NearbyRiskDto>();
            
        var data = await response.Content.ReadFromJsonAsync<List<NearbyRiskDto>>(cancellationToken);
        return data ?? new List<NearbyRiskDto>();
    }

    public async Task<double> CalculateDistanceAsync(int lat1, int lon1, int lat2, int lon2, CancellationToken cancellationToken = default)
    {
        var request = new 
        { 
