...
public interface IAlertWindowQueryService
{
    ...
    public Task<IReadOnlyList<AlertWindowDTO>> GetByRegionAsync(string region, CancellationToken cancellationToken = default);
    public Task<IReadOnlyList<AlertWindowDTO>> GetByStatusAsync(string status, CancellationToken cancellationToken = default);
}
